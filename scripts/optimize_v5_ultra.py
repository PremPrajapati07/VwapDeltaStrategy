# ============================================================
#  scripts/optimize_v5_ultra.py — Ultra-Fast Strategy Search
# ============================================================
import os
import sys
import pandas as pd
import datetime as dt
from itertools import product
from multiprocessing import Pool, cpu_count

# Add parent to path
sys.path.append(os.getcwd())

import config
import db
import arjun_model
import krishna_model as ml

# --- Global Constants for Worker Processes ---
DATA_SLICES = None
K_CHOICES   = None
ARJUN_MODEL = None

def worker_init(slices, choices, model):
    """Initializes each worker process with shared data once."""
    global DATA_SLICES, K_CHOICES, ARJUN_MODEL
    DATA_SLICES = slices
    K_CHOICES   = choices
    ARJUN_MODEL = model

def backtest_worker(params):
    """Blazing fast worker using pre-sliced daily data."""
    global DATA_SLICES, K_CHOICES, ARJUN_MODEL
    k_conf_limit, a_thresh, p_target, s_loss = params
    
    total_pnl = 0.0
    win_days = 0
    total_days = 0
    trade_count = 0
    daily_pnls = []
    
    # Iterate through days using pre-calculated slices
    for trade_date, choice in K_CHOICES.items():
        # Guard 1: Krishna Confidence
        if choice['confidence'] < k_conf_limit:
            continue
            
        # Access slice directly (no search overhead)
        strike_candles = DATA_SLICES.get(trade_date)
        if strike_candles is None: continue
        
        # Run optimized simulation
        day_trades = arjun_model.simulate_pnl_with_arjun_v2(
            strike_candles, 
            arjun_model=ARJUN_MODEL, 
            threshold=a_thresh,
            p_target=p_target,
            s_loss=s_loss
        )
        
        day_pnl = sum(t["pnl_pts"] for t in day_trades)
        total_pnl += day_pnl
        trade_count += len(day_trades)
        total_days += 1
        if day_pnl > 0: win_days += 1
        daily_pnls.append(day_pnl)

    win_rate = win_days / total_days if total_days > 0 else 0
    
    # Calculate Max DD
    max_dd = 0.0
    if daily_pnls:
        cp = pd.Series(daily_pnls).cumsum()
        max_dd = (cp.cummax() - cp).max()

    return {
        "k_conf": k_conf_limit, "a_thresh": a_thresh, "p_target": p_target, "s_loss": s_loss,
        "total_pnl": round(total_pnl, 1), "win_rate": round(win_rate, 3),
        "trade_count": trade_count, "avg_trades": round(trade_count/total_days, 1) if total_days > 0 else 0,
        "days_traded": total_days, "max_dd": round(max_dd, 1)
    }

def run_ultra_optimizer():
    print("\n🚀 ULTRA-FAST STRATEGY OPTIMIZER (v5) — FULL YEAR 2025")
    start_date = "2025-01-01"
    end_date   = "2025-12-31"

    with db.get_conn() as conn:
        feat_df = pd.read_sql(f"SELECT * FROM daily_features WHERE trade_date BETWEEN '{start_date}' AND '{end_date}'", conn)
        print("📦 Loading 2025 candles...")
        all_candles = pd.read_sql(f"""
            SELECT trade_date, strike, ts, straddle_price, vwap, ce_volume, pe_volume, 
                   ce_iv, pe_iv, ce_delta, pe_delta, ce_theta, pe_theta 
            FROM straddle_candles 
            WHERE trade_date BETWEEN '{start_date}' AND '{end_date}'
            ORDER BY ts, strike
        """, conn)

    all_candles = all_candles.rename(columns={"ts": "datetime"})
    model_k = ml.load_model()
    model_a = arjun_model.load_arjun_model()

    # 1. Pre-calculate Krishna & Slice Daily Data
    print("🍕 Slicing 3.5M candles into 247 daily portions...")
    slices = {}
    choices = {}
    days = sorted(feat_df['trade_date'].unique())
    
    for trade_date in days:
        day_feat = feat_df[feat_df['trade_date'] == trade_date]
        X_rows = day_feat[config.ML_FEATURES].fillna(0)
        proba = model_k.predict_proba(X_rows)[:, 1]
        best_idx = proba.argmax()
        strike = int(day_feat.iloc[best_idx]["strike"])
        
        choices[trade_date] = {"strike": strike, "confidence": float(proba[best_idx])}
        
        # Filter once per day manually (O(N) total)
        day_data = all_candles[(all_candles['trade_date'] == trade_date) & (all_candles['strike'] == strike)]
        if not day_data.empty:
            slices[trade_date] = day_data

    # 2. Def Grid (144 Combinations)
    grid = {
        "k_conf": [0.0, 0.35, 0.40, 0.45], 
        "a_thresh": [0.55, 0.60, 0.65], 
        "p_target": [80, 100, 150, 9999], 
        "s_loss": [-75, -100, -150]
    }
    combinations = list(product(*grid.values()))
    
    # 3. Parallel Search
    cores = cpu_count()
    print(f"🔥 Starting Ultra-Fast parallel search ({len(combinations)} iterations) on {cores} cores...")
    
    with Pool(processes=cores, initializer=worker_init, initargs=(slices, choices, model_a)) as pool:
        results = pool.map(backtest_worker, combinations)

    # 4. Results
    res_df = pd.DataFrame(results)
    # Win Rate > 80% primary target
    res_df["score"] = (res_df["total_pnl"] * (res_df["win_rate"]**2)) / (res_df["avg_trades"]**0.25).replace(0, 1)
    res_df = res_df.sort_values("score", ascending=False)
    
    res_path = os.path.join(config.LOGS_PATH, "optimization_v5_ultra_results.csv")
    res_df.to_csv(res_path, index=False)
    
    print("\n✅ ULTRA-FAST OPTIMIZATION COMPLETE")
    print(res_df.head(10).to_string(index=False))
    print(f"\nFinal Winners Log: {res_path}")

if __name__ == "__main__":
    run_ultra_optimizer()

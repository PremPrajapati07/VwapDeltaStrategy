...
def save_trading_state(strike, cum_pv, cum_vol, baseline_920=None):
    """Saves the current trading progress to disk using VWAP tracker metrics."""
    try:
        # Load existing state to preserve baseline if not provided
        existing = load_trading_state()
        final_baseline = baseline_920 or (existing.get("baseline_920") if existing else None)
...
def run_live_trading(lots: int = 1, paper_trade: bool = True, skip_kite: bool = False, skip_neo: bool = False):
    ...
    if should_resume and state:
        ...
        print(f"🤖 Running ML model (Krishna V2) for strike selection … (IST: {_now_ist().strftime('%H:%M:%S')})")
        
        snapshot_920      = dc.get_live_snapshot(data_broker, strikes)
        vwap_map          = vwap_tracker.get_all()

        # Capture 9:20 Baseline for future re-entries
        baseline_920 = {}
        for _, row in snapshot_920.iterrows():
            baseline_920[int(row["strike"])] = {
                "strike": int(row["strike"]),
                "ce_oi": float(row.get("oi_ce", 0)), "pe_oi": float(row.get("oi_pe", 0)),
                "straddle": float(row["straddle_price"]), "spot": float(row.get("synthetic_spot", spot))
            }

        try:
            prediction   = ml.predict_best_strike_v2(
                snapshot_df=snapshot_920, vwap_map=vwap_map,
                baseline_920=baseline_920, current_time=dt.time(9, 20),
            ...
            print(f"🎯 Model Prediction: {best_strike} | Confidence: {conf:.2%}")
        except Exception as e:
            # STRICT MODE: If model fails, we DO NOT fall back. Raise error.
            log.critical(f"STRICT MODE: ML prediction failed ({e}). Halting for safety.")
            raise RuntimeError(f"ML selection failed: {e}")

        save_trading_state(best_strike, 0.0, 0, baseline_920=baseline_920)
        ...
        # ── Exit logic ────────────────────────────────────────
            ...
            if position.sl_hit(straddle):
                ...
                if config.REENTRY_ALLOWED and config.REENTRY_REEVALUATE_STRIKE:
                    # RE-READY STRIKE FOR NEXT ENTRY
                    baseline = (state.get("baseline_920") if state else None) or (baseline_920 if 'baseline_920' in locals() else None)
                    selected_strike_info = _reevaluate_strike(
                        data_broker, strikes, vwap_tracker, vix,
                        nifty_prev_change, nifty_open_gap, today, expiry,
                        baseline_920=baseline, current_time=now.time()
                    )
                re_entry_count += 1
                continue
            ...
            if arjun is not None:
                ...
                if result["should_exit"]:
                    ...
                    if config.REENTRY_ALLOWED and config.REENTRY_REEVALUATE_STRIKE:
                        baseline = state.get("baseline_920") if state else baseline_920
                        selected_strike_info = _reevaluate_strike(
                            data_broker, strikes, vwap_tracker, vix,
                            nifty_prev_change, nifty_open_gap, today, expiry,
                            baseline_920=baseline, current_time=now.time()
...
def _reevaluate_strike(data_broker, strikes, vwap_tracker, vix,
                       ...
                       baseline_920, current_time):
    """STRICT MODE strike re-evaluation for re-entry."""
    snap     = dc.get_live_snapshot(data_broker, strikes)
    vwap_map = vwap_tracker.get_all()
    try:
        if not baseline_920:
             raise ValueError("baseline_920 missing for V2 model re-evaluation.")

        pred = ml.predict_best_strike_v2(
            snapshot_df=snap, vwap_map=vwap_map,
            baseline_920=baseline_920, current_time=current_time,
        ...
        log.critical(f"STRICT MODE: Re-entry ML failed ({e}). Halting re-entry.")
        raise RuntimeError(f"Re-entry ML failed: {e}")

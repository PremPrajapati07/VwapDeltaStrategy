...
MODEL_FILE   = os.path.join(config.MODELS_PATH, "strike_selector.pkl")
...
def train_model(feat_df: pd.DataFrame = None):
    ...
    daily = feat_df[feat_df["is_best"]].copy().sort_values("trade_date").reset_index(drop=True)

    le = LabelEncoder()
    # Use distance from ATM as the target label instead of absolute strike
    daily["label"] = le.fit_transform(daily["distance_from_atm"].astype(str))

    X = daily[config.ML_FEATURES].fillna(0)
    y = daily["label"]

    if len(y.unique()) < 2:
        log.warning("Only 1 type of best strike offset found. Accuracy metric may be invalid.")

    cv_scores = []
    # Only run CV if we have enough data (at least 20 rows/samples)
    if len(X) >= 20: 
        ...
        log.info("Not enough data for cross-validation. Training on full data.")
        print("💡 Not enough data for cross-validation. Training on full data.")
...
def predict_best_strike(snapshot_df: pd.DataFrame,
    ...
    # The model now outputs probabilities across available 'distance_from_atm' classes
    proba       = model.predict_proba(X)
    strike_list = feat_df["strike"].tolist()
    dist_list   = feat_df["distance_from_atm"].astype(str).tolist()

    scores = {s: 0.0 for s in strike_list}
    for idx, strike in enumerate(strike_list):
        dist_str = dist_list[idx]
        if dist_str in le.classes_:
            class_idx = list(le.classes_).index(dist_str)
            # The model predicts the probability of this offset being 'best'
            # based on the features at this specific strike
            scores[strike] = float(proba[idx][class_idx])

...
import krishna_v3_model as ml
import arjun_model_v4 as arjun_model
import live_trader_v3 as lt
...
def run_backtest(start_date=None, end_date=None, sl_multiplier=config.SL_MULTIPLIER):
    ...
    try:
        arjun     = arjun_model.load_arjun_v4()
        use_arjun = True
...
def _worker_backtest_day_v3(trade_date, day_feat, day_candles, use_ml, model, use_arjun, arjun, sl_multiplier):
    """
    Worker function for V3 Backtest (Krishna V3 + Arjun V4).
    Includes adaptive session features and advanced simulation.
    """
    if use_ml:
        # ✅ Fix: Dynamic & Adaptive Features injection for V3 compatibility
        ...
        # V3 adaptive/session
        day_feat["session_pnl_so_far"]   = 0.0
        day_feat["trades_taken_today"]   = 0
        day_feat["strike_last_pnl"]      = 0.0
        day_feat["strike_consec_losses"] = 0
        day_feat["session_is_positive"]  = 0
        day_feat["strike_is_virgin"]     = 1

        X_rows = day_feat[ml.V3_FEATURES].fillna(0)
        if X_rows.empty:
...
def run_backtest(start_date=None, end_date=None, sl_multiplier=config.SL_MULTIPLIER):
    ...
    print(f"🚀 Dispatching {len(tasks)} days to {max_workers} processes (Parallel V3 Mode) ...")

    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(_worker_backtest_day_v3, *task) for task in tasks]
